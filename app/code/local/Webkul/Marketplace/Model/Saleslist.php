		...
		*/
		...
		foreach ($order->getAllItems() as $item){
			...
			if(!$bundle_selection_attributes['option_id']){			
				...
				if(!Mage::getStoreConfig('mpadvancecommission/mpadvancecommission_options/usecommissionrule')) {		
					...
					if($advancecommission!=''){
						...
						if($commision>$totalamount){ $commission= $totalamount*(Mage::getStoreConfig('marketplace/marketplace_options/percent',$current_store))/100; }
					}	
				} else {
